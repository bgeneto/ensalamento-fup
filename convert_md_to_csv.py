...
class MarkdownToCSVConverter:
    ...
    def __init__(self):
        ...
        self.course_pattern = re.compile(
            r"^([A-Z]{3,9}\d{4})\s*/\s*([^/]+?)(?:\s*/\s*Turma\s*(\d+))?", re.IGNORECASE
        )
    ...
    def parse_html_table(
        self, table_html: str, room_name: str
        ...
        if first_row_has_colspan:
            ...
            if len(rows) > 1:
                ...
                print(
                    "    🐛 Debug: Skipping colspan header row, using row 2 for headers"
                )
            else:
                print(
                    "    🐛 Debug: Found colspan header but no second row for day headers"
                )
                return []
        else:
        ...
        print(
            f"    🐛 Debug: Found {len(day_headers)} valid day headers: {day_headers}"
        )
            ...
            for i, cell_content in enumerate(cells[1:]):
                ...
                if course_info:
                    ...
                    if block:
                        ...
                        print(
                            f"    🐛 Debug: Found course {course_info['codigo']} at {block}"
                        )
    ...
    def parse_markdown_file(
        ...
        for i, table_match in enumerate(table_matches):
            ...
            if colspan_match:
                ...
                print(
                    f"  📍 Processing table {i + 1}/{len(table_matches)}: {room_name} (inside header)"
                )
            ...
            if not room_name:
                ...
                room_matches = list(
                    re.finditer(r"Sala:\s*([A-Z0-9\-\/]+)", preceding_text)
                )
                ...
                for room_match in reversed(room_matches):
                    ...
                    if potential_room not in processed_rooms:
                        ...
                        print(
                            f"  📍 Processing table {i + 1}/{len(table_matches)}: {room_name} (outside table)"
                        )
                        break
            ...
            if room_name:
                ...
                if schedule_data:
                    ...
                    print("    ⚠️ No course allocations found")
    ...
    def generate_csv_data(
        self, schedule_data: Dict[str, List[Tuple[str, Dict[str, str]]]]
    ) -> List[List[str]]:
    ...
    def convert_to_csv(self, input_file: str, output_file: str):
        ...
        print(
            f"📈 Generated {len(csv_rows) - 1} time blocks with {sum(len(data) for data in schedule_data.values())} course allocations"
        )
...
def main():
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="docs/Ensalamento Convertido-2024.1.csv",
