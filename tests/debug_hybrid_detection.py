    ...
    with get_db_session() as session:
        ...
        for codigo, rooms in discipline_rooms.items():
            ...
            if has_multiple_rooms and has_non_classroom:
                candidates.append(
                    {"codigo": codigo, "room_ids": room_ids, "types": types_used}
                )
            elif has_multiple_rooms:
                print(
                    f"   {codigo}: {len(room_ids)} rooms, but all are classrooms (tipo=2)"
                )
        ...
        if candidates:
            ...
            print(
                "   - Disciplines with multiple rooms all use only classrooms (tipo=2)"
            )
