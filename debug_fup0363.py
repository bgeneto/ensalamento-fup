"""
Script para investigar por que FUP0363, apesar de ser híbrida,
...
def main():
    ...
    with get_db_session() as session:
        ...
        # 7. Verificar se o modo partial é utilizado
        print(f"\n\n{'='*80}")
        print(f"🔧 VERIFICAÇÃO DO MODO DE ALOCAÇÃO")
        print(f"{'='*80}\n")

        print("Para alocação SPLIT de disciplinas híbridas, é necessário usar:")
        print("   - execute_autonomous_allocation_partial() OU")
        print("   - Alocação manual por dia")
        print()
        print("O método execute_autonomous_allocation() padrão aloca TODOS os blocos")
        print("de uma demanda na MESMA sala, ignorando o comportamento híbrido.")
        ...
        if not is_hybrid:
            ...
            print("\n   🔍 Verificação crítica:")
            print(
                "   - O método execute_autonomous_allocation() PADRÃO NÃO suporta split!"
            )
            print("   - Para híbridas, usar execute_autonomous_allocation_partial()")
        elif len(rooms_used) == 0:
            print("❌ PROBLEMA IDENTIFICADO: Disciplina não foi alocada!")
            print("\n   Possíveis causas:")
            print("   1. Conflitos em todas as salas disponíveis")
            print("   2. Problema na execução da alocação")
            print("\n   Recomendação: Verificar logs de alocação")
        else:
